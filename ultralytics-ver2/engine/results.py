...
class Results(SimpleClass):
    ...
    def plot(
        ...
        if pred_boxes and show_boxes:
            ...
            #     name = ('' if id is None else f'id:{id} ') + names[c]
            #     label = (f'{name} {conf:.2f}' if conf else name) if labels else None
            #     annotator.box_distance_label(d.xyxy.squeeze(), HISTORY_VLA_pt[1], label, color=colors(c, True))
        ...
        if DRAW_LDWS:
            if l_p1 is not None and r_p1 is not None:
                ...
                if departure_distance>LD_TH:
                    ...
                    cv2.putText(im, text, (int(w/6.0), int(h/2.0)), cv2.FONT_HERSHEY_PLAIN,6, (0, 0, 255), 8, cv2.LINE_AA)
